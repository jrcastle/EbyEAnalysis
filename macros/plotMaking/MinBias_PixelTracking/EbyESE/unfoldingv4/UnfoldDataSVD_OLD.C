...
void UnfoldDataSVD(){
  ...
  setTDRStyle();
  TFile * fsave = new TFile(Form("/home/j550c590/CMSSW_7_5_8_patch2/src/HeavyIonsAnalysis/EbyEAnalysis/macros/unfolding/txt/PbPb_2015/data%i.root", VN), "recreate");

  //-- Get histos from datafile
  TFile * fData = new TFile("/home/j550c590/CMSSW_7_5_8_patch2/src/HeavyIonsAnalysis/EbyEAnalysis/macros/unfolding/data/PbPb_2015/CastleEbyE.root");
  ...
  fShift = new TF1("fShift", "TMath::Sqrt( pow([0],2) + pow([1],2) + 2.*[0]*[1]*TMath::Cos(x) )", 0, 2.*TMath::Pi());
  ...
  for(int c = 0; c < NCENT; c++){
	...
	for ( int j = 1; j <= NBins[ptBin][c]; j++ ) {
	  double w = 1.;
	  if ( sw ) {
	    w = hw[c]->GetBinContent(j); //-- w = hw[c]->GetBinContent(j);
	  }
    ...
    hreco0[c]->SetName(Form("hreco0_%i_%i", VN, c));

    svdUnf[c]  = (TSVDUnfold*) unfold0[c]->Impl();
    hdi[c]     = (TH1D*) svdUnf[c]->GetD();
    hdi[c]->SetName(Form("hdi_%i_%i", VN, c));
    ...
    for(int ik = kreg-4; ik <= kreg+4; ik++){
      ...
      hrecokreg[dummy][c]->SetName(Form("hrecokreg%i_%i_%i", ik, VN, c));
      dummy++;
    }
    ...
    hdi[c]->Write();
    hreco0[c]->Write();
    for(int i=0; i<9; i++) hrecokreg[i][c]->Write();
  }
