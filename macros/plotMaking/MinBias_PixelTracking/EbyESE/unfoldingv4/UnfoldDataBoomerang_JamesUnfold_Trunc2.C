...
void UnfoldDataBoomerang_JamesUnfold_Trunc2( int nord ){
  ...
  for(int c = 0; c < NCENT; c++){
    ...
    for(int i = 0; i < NITER; i++){
      ...
      if( smoothIter ) unfolder.SetSmoothIter();
      unfolder.Unfold(h1D[c], iter[i]);
      hreco[c][i]   = (TH1D*) unfolder.GetHReco( Form("hreco%i_c%i", iter[i], c) );
      hrefold[c][i] = (TH1D*) unfolder.refold( hreco[c][i], Form("hrefold%i_c%i", iter[i], c) );
      hrefold[c][i]->GetXaxis()->SetTitle( Form("v_{%i}", norder_) );
      hrefold[c][i]->GetYaxis()->SetTitle( "Events" );
    }
