...
#include "/home/j550c590/CMSSW_7_5_8_patch2/src/HeavyIonsAnalysis/EbyEAnalysis/macros/unfolding/DAgostiniUnfold/DAgostiniUnfold.h"
#include "TLatex.h"
#include "TFile.h"
#include "TH1I.h"
#include "TH1D.h"
...
void UnfoldMCEbyE_JamesUnfold( int ipt ){
  ...
  for(int c = 0; c < NCENT; c++){
    ...
    for(int i = 0; i < NITER; i++){
      ...
      hreco[c][i]   = (TH1D*) unfolder.GetHReco( Form("hreco%i_%i_%i", iter[i], VN, c) );
      hrefold[c][i] = (TH1D*) unfolder.refold( hreco[c][i], Form("hrefold%i_%i_%i", iter[i], VN, c) );
    }  
