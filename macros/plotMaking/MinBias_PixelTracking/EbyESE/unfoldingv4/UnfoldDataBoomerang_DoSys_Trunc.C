...
void UnfoldDataBoomerang_DoSys_Trunc( int nord ){
  ...
  for(int c = 0; c < NCENT; c++){
    ...
    if(GaussResp){
      ...
      hresp[c] = new TH2D( Form("hresp_c%i", c), Form("hresp_c%i", c), NBinsV2[c], 0., v2Max[c], NBinsV2[c], 0., v2Max[c]);
    ...
    for(int i = 0; i < NITER; i++){ 
      ...
      hCovMat[c][i] = new TH2D(Form("hCovMat%i_c%i", iter[i], c), Form("hCovMat%i_c%i", iter[i], c), NBinsV2[c], 0., v2Max[c], NBinsV2[c], 0., v2Max[c]);
