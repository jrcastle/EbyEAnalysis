...
void UnfoldDataEbyESE_JamesUnfold( int nord ){
  ...
  for(int c = 0; c < NCENT; c++){
	...
	//if(c != centbin || iEP != EPSymmBin) continue;
	if(iEP != EPSymmBin) continue; 
	std::cout<< "!! Processing EP " << EPSymmNames[iEP] << "\tcent bin = " << c << "\t qbin = " << iqn << std::endl;
	...
	for(int i = 0; i < NITER; i++){
	  if( smoothResp ) hresp[c][iEP][iqn]->Smooth(1, "kb5");
	  DAgostiniUnfold unfolder(hresp[c][iEP][iqn]);
	  if( smoothIter ) unfolder.SetSmoothIter();
	  unfolder.Unfold(h1D[c][iEP][iqn], iter[i]);
	  hreco[c][iEP][iqn][i]   = (TH1D*) unfolder.GetHReco( Form("hreco%i_%s_c%i_qbin%i", iter[i], EPSymmNames[iEP].data(), c, iqn) );
	  hrefold[c][iEP][iqn][i] = (TH1D*) unfolder.refold( hreco[c][iEP][iqn][i], Form("hrefold%i_%s_c%i_qbin%i", iter[i], EPSymmNames[iEP].data(), c, iqn) );
	  hrefold[c][iEP][iqn][i]->GetXaxis()->SetTitle( Form("v_{%i}", norder_) );
	  hrefold[c][iEP][iqn][i]->GetYaxis()->SetTitle( "Events" );
	}
