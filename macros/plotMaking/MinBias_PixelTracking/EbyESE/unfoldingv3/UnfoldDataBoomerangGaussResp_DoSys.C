...
void UnfoldDataBoomerangGaussResp_DoSys( int nord ){
  ...
  for(int c = 0; c < NCENT; c++){
    ...
    for(int i = 0; i < NITER; i++){ 
      ...
      hCovMat[c][i] = new TH2D(Form("hCovMat%i_c%i", iter[i], c), Form("hCovMat%i_c%i", iter[i], c), NBins, 0., vnMax[norder_], NBins, 0., vnMax[norder_]);
