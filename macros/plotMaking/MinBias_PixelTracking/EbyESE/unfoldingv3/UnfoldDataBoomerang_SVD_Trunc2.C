...
void UnfoldDataBoomerang_SVD_Trunc2( int nord ){
  ...
  for(int c = 0; c < NCENT; c++){
    ...
    for(int ik = kreg-4; ik <= kreg+4; ik++){
      ...
      hCovMat[c][dummy] = new TH2D(Form("hCovMatkreg%i_c%i", dummy, c), Form("hCovMatkreg%i_c%i", dummy, c), NBinsV2[c], 0., v2Max[c], NBinsV2[c], 0., v2Max[c]);
    ...
    */
    for(int ik = 1; ik <= NKREG; ik++){
      ...
      hCovMat[c][ik-1] = new TH2D(Form("hCovMatkreg%i_c%i", ik, c), Form("hCovMatkreg%i_c%i", ik, c), NBinsV2[c]/2, 0., v2Max[c], NBinsV2[c]/2, 0., v2Max[c]);
