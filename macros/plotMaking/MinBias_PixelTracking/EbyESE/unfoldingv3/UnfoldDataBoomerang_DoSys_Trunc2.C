...
void UnfoldDataBoomerang_DoSys_Trunc2( int nord ){
  ...
  for(int c = 0; c < NCENT; c++){
    ...
    if(GaussResp){
      ...
      hresp[c] = new TH2D( Form("hresp_c%i", c), Form("hresp_c%i", c), NBinsV2[c], 0., v2Max[c], NBinsV2[c]/2, 0., v2Max[c]);
      hresp[c]->GetXaxis()->SetTitle( Form("v_{%i}^{obs}", norder_) );
      hresp[c]->GetYaxis()->SetTitle( Form("v_{%i}^{true}", norder_) );
      hresp[c]->SetOption("colz");

      f1Dx[c] = new TF1( Form("f1Dx_c%i", c), "gaus", -v2Max[c], v2Max[c] );
      f1Dy[c] = new TF1( Form("f1Dy_c%i", c), "gaus", -v2Max[c], v2Max[c] );
    ...
    if(StudTresp){
      ...
      f1Dx_StudT[c] = new TF1(Form("f1Dx_StudT_c%i", c), "[0] * pow([3]/( [3]+pow( (x-[1])/[2], 2) ), (1.+[3])/2.)  / ( TMath::Sqrt([3]) * [2] * TMath::Beta([3]/2.,0.5) )", -v2Max[c], v2Max[c]);
      ...
      f1Dy_StudT[c] = new TF1(Form("f1Dy_StudT_c%i", c), "[0] * pow([3]/( [3]+pow( (x-[1])/[2], 2) ), (1.+[3])/2.)  / ( TMath::Sqrt([3]) * [2] * TMath::Beta([3]/2.,0.5) )", -v2Max[c], v2Max[c]);
    ...
    for(int i = 0; i < NITER; i++){ 
      ...
      TMatrixD covMat = unfolder.Ereco();

      hCovMat[c][i] = new TH2D(Form("hCovMat%i_c%i", iter[i], c), Form("hCovMat%i_c%i", iter[i], c), NBinsV2[c]/2, 0., v2Max[c], NBinsV2[c]/2, 0., v2Max[c]);
      hCovMat[c][i]->GetXaxis()->SetTitle( Form("v_{%i}^{Unfold}", norder_) );
      hCovMat[c][i]->GetYaxis()->SetTitle( Form("v_{%i}^{Unfold}", norder_) );
      hCovMat[c][i]->SetOption("colz");
      M2H(covMat, hCovMat[c][i]);

      hreco[c][i]->SetName(Form("hreco%i_c%i", iter[i], c) );
      hrefold[c][i] = (TH1D*) response[c]->ApplyToTruth( hreco[c][i] );
      hrefold[c][i]->SetName( Form("hrefold%i_c%i", iter[i], c) );
    }
