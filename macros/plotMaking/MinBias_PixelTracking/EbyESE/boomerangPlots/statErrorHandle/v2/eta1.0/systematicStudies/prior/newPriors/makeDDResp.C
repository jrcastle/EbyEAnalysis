...
void makeDDResp(){
  ...
  TH1D * hVnObsThin[NCENT][NSPLIT];
  TH1D * hVnObsWide[NCENT][NSPLIT];
  ...
  for(int iS = 0; iS < NSPLIT; iS++){
    ...
    if( !thinner_ && !wider_ ) fOut[iS] = new TFile( Form("dataDrivenResponseAndPriors_Split%i.root", iS), "recreate");
    if( thinner_ && !wider_ )  fOut[iS] = new TFile( Form("dataDrivenResponseAndPriors_Thinner_Split%i.root", iS), "recreate");
    if( wider_ && !thinner_ )  fOut[iS] = new TFile( Form("dataDrivenResponseAndPriors_Wider_Split%i.root", iS), "recreate");
    ...
    for(int icent = 0; icent < NCENT; icent++){
      ...
      hVnObsThin[icent][iS] = (TH1D*) fAna[iS]->Get( Form("qwebye/hVnFull_Thinner_c%i", icent) );
      hVnObsWide[icent][iS] = (TH1D*) fAna[iS]->Get( Form("qwebye/hVnFull_Wider_c%i", icent) );

      if( !thinner_ && !wider_ ) hPrior[icent][iS] = (TH1D*) hVnObs[icent][iS]->Clone( Form("hPrior_c%i", icent) );
      if( thinner_ && !wider_ )  hPrior[icent][iS] = (TH1D*) hVnObsThin[icent][iS]->Clone( Form("hPrior_c%i", icent) );
      if( wider_ && !thinner_ )  hPrior[icent][iS] = (TH1D*) hVnObsWide[icent][iS]->Clone( Form("hPrior_c%i", icent) );
