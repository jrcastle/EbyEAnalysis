...
void SmoothSys(int n = 2){
  ...
  if( fSysReg->IsZombie() || fSysResp->IsZombie() || fSysNewCC->IsZombie() || fSysTkQ->IsZombie() || fSysVtx->IsZombie() ){
    std::cout << "WARNING! Systematic studies not run... \n" 
	      << "Please run systematic studies first and then come back to this macro."
	      << " Exiting now..."
	      << std:: endl;
    exit(0);
  }
