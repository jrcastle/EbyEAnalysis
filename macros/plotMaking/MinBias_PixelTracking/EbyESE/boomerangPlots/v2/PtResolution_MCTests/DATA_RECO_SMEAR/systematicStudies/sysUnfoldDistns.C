...
void sysUnfoldDistns(){
  ...
  */
  ...
  leg->SetFillStyle(0);
  leg->SetBorderSize(0);
  leg->AddEntry(hObs[c], Form("v_{%i}^{obs}", norder_), "lp");
  for(int i = 0; i < NITER; i++) leg->AddEntry(hRefold[c][i], Form("D'Agostini iter = %i", iter[i]), "l");

  TLine * l1p2 = new TLine(grRefoldChi2[c]->GetXaxis()->GetXmin(), 1.2, grRefoldChi2[c]->GetXaxis()->GetXmax(), 1.2);
  l1p2->SetLineStyle(2);
  l1p2->SetLineWidth(2);
