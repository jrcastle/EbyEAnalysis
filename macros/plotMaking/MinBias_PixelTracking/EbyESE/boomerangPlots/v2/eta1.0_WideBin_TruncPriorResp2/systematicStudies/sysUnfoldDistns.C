...
void sysUnfoldDistns(){
  ...
  fUnfold = new TFile( Form("../UnfoldResults/dataResp/data%i_CovMatrix.root", norder_) );
  /*
  ...
  */
  ...
  for(int i = 0; i < NITER; i++) leg->AddEntry(hRefold[0][i], Form("D'Agostini iter = %i", iter[i]), "l");
  ...
  for(int icent = 0; icent < NCENT; icent++){
    ...
    hObs[icent]->SetMaximum(hFinalUnf[icent]->GetMaximum()*2.);
    hObs[icent]->Draw();
