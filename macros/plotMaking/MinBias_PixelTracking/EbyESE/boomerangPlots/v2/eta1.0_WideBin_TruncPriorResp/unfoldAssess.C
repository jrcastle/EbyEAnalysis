...
void unfoldAssess(){
  ...
  TFile * fUnfold;
  TH2D * hResp[NCENT];
  TH1D * hSmear[NCENT][NBinsV2[NCENT-1]];
  double v2True[NCENT][NBinsV2[NCENT-1]];
  double smearWidth[NCENT][NBinsV2[NCENT-1]];
  TGraph * grSmearWidth[NCENT];
  ...
  //-- initialize v2true and smearWidth arrays...
  for(int icent = 0; icent < NCENT; icent++){
    for(int iv2 = 0; iv2 < NBinsV2[NCENT-1]; iv2++){
      v2True[icent][iv2]     = iv2*binw;
      smearWidth[icent][iv2] = 0.;
    }
  }
  ...
  for(int icent = 0; icent < NCENT; icent++){
	...
	h2Vn2D0v1_X[icent]->Fit(Form("fStudTX_c%i", icent), "L0", "", h2Vn2D0v1_X[icent]->GetMean() - sigmaFit * h2Vn2D0v1_X[icent]->GetRMS(), h2Vn2D0v1_X[icent]->GetMean() + sigmaFit * h2Vn2D0v1_X[icent]->GetRMS());
      ...
      h2Vn2D0v1_X[icent]->GetYaxis()->SetTitleOffset(1.2);
      ...
      h2Vn2D0v1_Y[icent]->GetYaxis()->SetTitleOffset(1.2);
      ...
      h2Vn2D0v1[icent]->GetYaxis()->SetTitleOffset(1.2);
      ...
      for(int iv2 = 0; iv2 < NBinsV2[icent]; iv2++){
	hSmear[icent][iv2]     = hResp[icent]->ProjectionX(Form("hSmear%i_c%i", iv2, icent), iv2, iv2);
	v2True[icent][iv2]     = hSmear[icent][iv2]->GetBinCenter(iv2);
	smearWidth[icent][iv2] = hSmear[icent][iv2]->GetRMS();
      }
      grSmearWidth[icent] = new TGraph(NBinsV2[icent], v2True[icent], smearWidth[icent]);
      formatGraph(grSmearWidth[icent], "v_{2}^{True}", 0., 0.08, "Smear Width", 1, 20, Form("grSmearWidth_c%i", icent));
      grSmearWidth[icent]->GetXaxis()->SetRangeUser(0., v2Max[icent]);
      grSmearWidth[icent]->GetXaxis()->SetNdivisions(507);
  ...
  int c;
  TLegend * leg2SE = new TLegend(0.2, 0.75, 0.6, 0.95);
  legInit( leg2SE );
  ...
  latex.DrawLatex(0.2, 0.2, Form("Cent %i - %i%s", cent_min[c], cent_max[c], "%") );
  ...
  c2SE->cd(4);
  h2Vn2D0v1[c]->Draw();
  latex.DrawLatex(0.65,0.88, Form("Cent %i - %i%s", cent_min[c], cent_max[c], "%") );
  ...
  //-- smear width as a function of v2 true
  TCanvas * cSmearWidth = new TCanvas("cSmearWidth", "cSmearWidth", 2000, 1500);
  cSmearWidth->Divide(4,3);

  for(int icent = 0; icent < NCENT; icent++){
    cSmearWidth->cd(icent+1);
    grSmearWidth[icent]->Draw("ap");
    double maxsig = TMath::MaxElement(NBinsV2[icent], grSmearWidth[icent]->GetY());
    std::cout << "Cent = " << icent << "\tMax Sigma = " << Form("%.3f", maxsig) << std::endl;
    latex.DrawLatex(0.2, 0.88, Form("Cent %i - %i%s", cent_min[icent], cent_max[icent], "%") );
  }
  cSmearWidth->SaveAs("plots/unfolding/smearWidth.pdf");
