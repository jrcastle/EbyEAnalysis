...
void svdUnfoldResults(){
  ...
  */
  ...
  /*
  hKreg->SetBinContent(0, 4);
  hKreg->SetBinContent(1, 3);
  hKreg->SetBinContent(2, 5);
  hKreg->SetBinContent(3, 5);
  hKreg->SetBinContent(4, 5);
  hKreg->SetBinContent(5, 5);
  hKreg->SetBinContent(6, 5);
  hKreg->SetBinContent(7, 5);
  hKreg->SetBinContent(8, 5);
  hKreg->SetBinContent(9, 5);
  hKreg->SetBinContent(10, 5);
  hKreg->SetBinContent(11, 8);
  */
  for(int icent = 0; icent < NCENT; icent++){

    std::cout << Form("============== c%i ==============", icent) << std::endl;
    ...
    double lmax = 25;
    Kreg[icent] = hKreg->GetBinContent(icent+1);
    int kreg = Kreg[icent];
    if(kreg == 0) kreg = 1;
    ...
    //-- refold
    for(int ik = 0; ik < NKREG; ik++){
      //-- Grab the cov matrix
      hCovMatrix[icent][ik] = (TH2D*) fSVD->Get( Form("hCovMatkreg%i_c%i", ik+1, icent) );

      //-- Refold
      hrefold[icent][ik] = (TH1D*) fSVD->Get( Form("hrefoldkreg%i_c%i", ik+1, icent) );
      hrefold[icent][ik]->SetLineColor(centCol[ik]);
      hrefold[icent][ik]->SetLineWidth(2);

      //-- Unfold
      finalUnf[icent][ik] = (TH1D*) fSVD->Get( Form("hrecokreg%i_c%i", ik+1, icent) );
      finalUnf[icent][ik]->GetXaxis()->SetTitle( Form("v_{%i}", norder_) );
      finalUnf[icent][ik]->SetLineColor(centCol[ik]);
      finalUnf[icent][ik]->SetMarkerColor(centCol[ik]);
      if( ik+1 == kreg ){ 
	finalUnf[icent][ik]->SetMarkerStyle(21);
	finalUnf[icent][ik]->SetMarkerColor(1);
	finalUnf[icent][ik]->SetLineColor(1);
	hrefold[icent][ik]->SetLineColor(1);
      }
      else               finalUnf[icent][ik]->SetMarkerStyle(24);
      //FixUnfold( finalUnf[icent] );
    }

    std::cout << kreg<<std::endl;
    EbyECumu cumu( finalUnf[icent][kreg-1] );
  ...
  */
  ...
  for(int icent = 0; icent < NCENT; icent++){
    int kreg = Kreg[icent];
    if(kreg == 0) kreg = 1;
    legUnf[icent] = new TLegend(0.1283, 0.2440, 0.8108, 0.8539);
    legUnf[icent]->SetBorderSize(0);
    legUnf[icent]->SetFillStyle(0);
    legUnf[icent]->AddEntry(hObs[0],       "Observed", "lp");
    for(int ik = 0; ik < NKREG; ik++) legUnf[icent]->AddEntry(finalUnf[icent][ik], Form("Unfolded, kreg = %i", ik+1),"lp");
    ...
    hObs[icent]->Draw();
    for(int ik = 0; ik < NKREG; ik++) finalUnf[icent][ik]->Draw("same");
    finalUnf[icent][kreg-1]->Draw("same");
    latex.DrawLatex(0.53, 0.89, "Unfolded Distribution");
    latex.DrawLatex(0.18, 0.89, "B");

    //-- Convert cov matrix to corr matrix
    hCorrMatrix[icent] = new TH2D( Form("hCorrMatrix_c%i", icent), Form("hCorrMatrix_c%i", icent), NBinsV2[icent], 0., v2Max[icent], NBinsV2[icent], 0., v2Max[icent] );
    ...
    TMatrixD MCov = H2M( hCovMatrix[icent][kreg-1] );
    TMatrixD S(NBinsV2[icent], NBinsV2[icent]);
    for(int i = 0; i<NBinsV2[icent]; i++){
      S(i,i) = sqrt( MCov(i,i) );
    }
    TDecompSVD svd(S);
    ...
    hObs[icent]->Draw();
    for(int ik = 0; ik < NKREG; ik++) hrefold[icent][ik]->Draw("same");
    hObs[icent]->Draw("same");
    hrefold[icent][kreg-1]->Draw("same");
