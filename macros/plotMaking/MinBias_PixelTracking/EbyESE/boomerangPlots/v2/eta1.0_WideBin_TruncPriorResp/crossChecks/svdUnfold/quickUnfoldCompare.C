...
void quickUnfoldCompare(){
  ...
  for(int icent = 0; icent < NCENT; icent++){
    ...
    for(int ik = 0; ik < NKREG; ik++){
      hUnfoldSVD[icent][ik] = (TH1D*) fSVD->Get( Form("hrecokreg%i_c%i", ik+1, icent) );
      hUnfoldSVD[icent][ik]->SetLineColor(2);
      hUnfoldSVD[icent][ik]->SetMarkerColor(2);
      hUnfoldSVD[icent][ik]->GetXaxis()->SetNdivisions(508);

      hUnfoldSVD_RatioToDAG[icent][ik] = (TH1D*) hUnfoldSVD[icent][ik]->Clone( Form("hUnfoldSVD_RatioToDAGkreg%i_c%i", ik+1, icent) );
      ...
      hRefoldSVD[icent][ik] = (TH1D*) fSVD->Get( Form("hrefoldkreg%i_c%i", ik+1, icent) );
      double chi2 = hRefoldSVD[icent][ik]->Chi2Test(hObs[icent], "CHI2/NDF");

      if(ik == kreg - 1){
        //std::cout<<"Cent = "<<icent<<"\tkreg = "<<ik+1<<"\tchi2 = "<<chi2<<"\t******"<<std::endl;
        SVDchi2[icent] = chi2;
      }
      /*
      if(chi2 <1.2){
	SVDchi2[icent] = chi2;
        hKreg->SetBinContent(icent+1, ik+1);
        break;
      }
      if(ik == NKREG-1){
        SVDchi2[icent] = chi2;
  ...
  for(int icent = 0; icent < NCENT; icent++) std::cout << "Cent " << icent <<"\t" << Form("%.2f", DAGchi2[icent]) << "\t" << Form("%.2f", SVDchi2[icent]) << "\t" << Form("%.2f", DAGchi2[icent] - SVDchi2[icent]) << std::endl;

  //TLegend * legC = new TLegend(0.6, 0.2, 0.9, 0.4);
  TLegend * legC = new TLegend(0.2, 0.7, 0.5, 0.9);
  legInit( legC );
  legC->AddEntry(hUnfoldDAG[0][finalIter[0]], "D'Agostini", "lp");
  legC->AddEntry(hUnfoldSVD[0][0],            "SVD",        "lp");

  //-- Draw ch2------------
  TGraph * grChi2SVD = new TGraph(NCENT, centBinCenter, SVDchi2);
  formatGraph(grChi2SVD, "Centrality %", 0.1, 10., "#chi^{2}/NDF", 2, 20, "grChi2SVD");
  TGraph * grChi2DAG = new TGraph(NCENT, centBinCenter, DAGchi2);
  formatGraph(grChi2DAG, "Centrality %", 0.1, 10., "#chi^{2}/NDF", 1, 20, "grChi2DAG");

  TLine * lone = new TLine(0, 1., grChi2DAG->GetXaxis()->GetXmax(), 1.);

  TCanvas * cChi2 = new TCanvas("cChi2","cChi2", 500, 500);
  cChi2->cd();
  grChi2SVD->Draw("ap");
  grChi2DAG->Draw("psame");
  legC->Draw("same");
  lone->Draw("same");
  ...
  for(int icent = 0; icent < NCENT; icent++){
    ...
    if(kreg >= NKREG) kreg = NKREG-1;
    std::cout<<kreg<<"\t"<<NKREG<<"\t"<<hUnfoldSVD[icent][kreg]->GetName()<<std::endl;
    c->cd(icent+1);
  ...
  for(int icent = 0; icent < NCENT; icent++){
    ...
    latex.DrawLatex(0.2, 0.2, Form("Cent %i - %i%s", cent_min[icent], cent_max[icent], "%"));
    latex.DrawLatex(0.2, 0.25, Form("kreg = %i", kreg+1));
  }
