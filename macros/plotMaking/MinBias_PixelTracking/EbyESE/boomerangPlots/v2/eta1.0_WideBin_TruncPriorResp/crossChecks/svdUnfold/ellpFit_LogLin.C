...
void ellpFit_LogLin(){
  ...
  double vnmax[NCENT]   = {0.3,   0.3,   0.3,   0.3,   0.3,  0.3,  0.3,  0.3,  0.3,  0.3,  0.25,  0.23};

  TGraph * grChi2DAG;
  TGraph * grChi2SVD;
  double chi2DAG[NCENT];
  double chi2SVD[NCENT];
  ...
  for(int icent = 0; icent < NCENT; icent++){
    ...
    if( TRUNC_ ) FixUnfold( hUnfoldDAG[icent] );
    ...
    fEllP_DAG[icent]->SetParameters(e0Guess[icent], alGuess[icent], knGuess[icent], hUnfoldDAG[icent]->GetMaximum());
    fEllP_DAG[icent]->SetParNames("ecc0", "alpha", "kn", "Scale");

    hUnfoldDAG[icent]->Fit( Form("fEllP_DAG_c%i", icent), "L0", "", 0.0, vnmax[icent]);
    ...
    chi2DAG[icent] = fEllP_DAG[icent]->GetChisquare() /fEllP_DAG[icent]->GetNDF();
    ...
    if(TRUNC_ ) FixUnfold( hUnfoldSVD[icent] );
    ...
    fEllP_SVD[icent]->SetParameters(e0Guess[icent], alGuess[icent], knGuess[icent], hUnfoldSVD[icent]->GetMaximum());
    fEllP_SVD[icent]->SetParNames("ecc0", "alpha", "kn", "Scale");

    hUnfoldSVD[icent]->Fit( Form("fEllP_SVD_c%i", icent), "L0", "", 0.0, vnmax[icent]);
    ...
    chi2SVD[icent] = fEllP_SVD[icent]->GetChisquare() /fEllP_SVD[icent]->GetNDF();
  ...
  cFits_DAG->SaveAs("cFits_DAG.pdf");
  //cFits_SVD->SaveAs("../plots/systematicStudies/cFits_SVD.pdf");

  grChi2DAG = new TGraph(NCENT, centBinCenter, chi2DAG);
  formatGraph(grChi2DAG, "Centrality %", 0.1, 500, "#chi^{2}/NDF", 1, 20, "grChi2DAG");
  grChi2SVD = new TGraph(NCENT,centBinCenter, chi2SVD);
  formatGraph(grChi2SVD, "Centrality %", 0.1, 500, "#chi^{2}/NDF", 2, 20, "grChi2SVD");

  TLegend * l = new TLegend(0.19, 0.82, 0.39, 0.93);
  legInit( l );
  l->AddEntry(grChi2DAG, "D'Agostini", "lp");
  l->AddEntry(grChi2SVD, "SVD", "lp");

  TCanvas * c = new TCanvas("c", "c", 500, 500);
  c->cd();
  c->SetLogy();
  grChi2DAG->Draw("ap");
  grChi2SVD->Draw("psame");
  l->Draw("same");
