...
void sysUnfoldDistns(){
  ...
  TH1D * hSysErrPointToPoint[NCENT];
  TF1 * fSmooth[NCENT];
  double vnMax[NCENT];
  ...
  for(int icent = 0; icent < NCENT; icent++){
    ...
    hSysErrPointToPoint[icent]        = (TH1D*) hFinalUnf[icent]->Clone( Form("hSysErrPointToPoint_c%i", icent) ); // for smoothing
    ...
    hSysErrPointToPoint[icent]        -> Reset();
    ...
    hSysErrPointToPoint[icent]->SetMaximum(0.01);

    hSysReg_RatioToNominal[icent]     -> SetLineColor(2);
    hSysVtx3_RatioToNominal[icent]    -> SetLineColor(4);
    hSysVtx3_15_RatioToNominal[icent] -> SetLineColor(4);
    hSysTkLoose_RatioToNominal[icent] -> SetLineColor(8);
    hSysTkTight_RatioToNominal[icent] -> SetLineColor(8);

    hSysReg_RatioToNominal[icent]     -> SetMarkerColor(2);
    hSysVtx3_RatioToNominal[icent]    -> SetMarkerColor(4);
    hSysVtx3_15_RatioToNominal[icent] -> SetMarkerColor(4);
    hSysTkLoose_RatioToNominal[icent] -> SetMarkerColor(8);
    hSysTkTight_RatioToNominal[icent] -> SetMarkerColor(8);
    ...
    for(int i = 1; i <= NBins; i++){
      ...
      //double relErrTot = sqrt( pow(relErr_Reg, 2) );
      //double relErrTot = sqrt( pow(relErr_Resp, 2) );
      //double relErrTot = sqrt( pow(relErr_Vtx, 2) );
      //double relErrTot = sqrt( pow(relErr_TkQuality, 2) );
      //double relErrTot = 0.1;
      ...
      //hSysErrPointToPoint[icent]->SetBinContent(i, sysErrTot/hFinalUnf[icent]->GetBinContent(i) );
      hSysErrPointToPoint[icent]->SetBinContent(i, sysErrTot );
    ...
    if(smooth){
      hSysErrPointToPoint[icent]->Smooth();
      for(int i = 1; i <= NBins; i++){
	if( hFinalUnf[icent]->GetBinContent(i) == 0 ) continue;
	double sys  = hSysErrPointToPoint[icent]->GetBinContent(i);
	double stat = hFinalUnf[icent]->GetBinError(i);
	double tot = sqrt( sys*sys + stat*stat );
    ...
    hFinalUnfoldSys[icent]->SetFillColor(2);
    hFinalUnfoldSys[icent]->GetXaxis()->SetRange(1, hFinalUnfoldSys[icent]->FindBin(0.2));
    hFinalUnfoldSys[icent]->Draw("e2");
    setex2->Draw();
    hFinalUnf[icent]->Draw("same");
    latex.DrawLatex(0.67, 0.88, Form( "Cent %i-%i%s", cent_min[icent], cent_max[icent], "%") );

    cSysPointToPoint->cd(icent+1);
    hSysErrPointToPoint[icent]->Draw();
    //fSmooth[icent]->Draw("same");
    latex.DrawLatex(0.67, 0.88, Form( "Cent %i-%i%s", cent_min[icent], cent_max[icent], "%") );

  } // End Cent loop

  cSysReg->SaveAs("../plots/systematicStudies/sysReg_UnfoldDistn.pdf");
  cSysVtx->SaveAs("../plots/systematicStudies/sysVtx_UnfoldDistn.pdf");
  cSysTkQ->SaveAs("../plots/systematicStudies/sysTkQ_UnfoldDistn.pdf");
  cFinalUnfold->SaveAs("../plots/systematicStudies/sysFinalUnfoldDistns.pdf");
