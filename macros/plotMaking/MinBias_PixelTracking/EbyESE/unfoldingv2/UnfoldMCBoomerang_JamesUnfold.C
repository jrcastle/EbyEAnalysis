...
void UnfoldMCBoomerang_JamesUnfold( int nord ){
  ...
  for(int c = 0; c < NCENT; c++){
    ...
    if(GaussResp){
      ...
      hresp[c] = new TH2D( Form("hresp_c%i", c), Form("hresp_c%i", c), NBins, 0., vnMax, NBins, 0., vnMax);
      hresp[c]->GetXaxis()->SetTitle( Form("v_{%i}^{obs}", norder_) );
      hresp[c]->GetYaxis()->SetTitle( Form("v_{%i}^{true}", norder_) );
      hresp[c]->SetOption("colz");

      f1Dx[c] = new TF1( Form("f1Dx_c%i", c), "gaus", -vnMax, vnMax );
      f1Dy[c] = new TF1( Form("f1Dy_c%i", c), "gaus", -vnMax, vnMax );
    ...
    if(StudTresp){
      ...
      f1Dx_StudT[c] = new TF1(Form("f1Dx_StudT_c%i", c), "[0] * pow([3]/( [3]+pow( (x-[1])/[2], 2) ), (1.+[3])/2.)  / ( TMath::Sqrt([3]) * [2] * TMath::Beta([3]/2.,0.5) )", -vnMax, vnMax);
      ...
      f1Dy_StudT[c] = new TF1(Form("f1Dy_StudT_c%i", c), "[0] * pow([3]/( [3]+pow( (x-[1])/[2], 2) ), (1.+[3])/2.)  / ( TMath::Sqrt([3]) * [2] * TMath::Beta([3]/2.,0.5) )", -vnMax, vnMax);
